...
test_image = "../Sealion/Train/4.jpg"
...
# load traned net
model = create_model("./cp/11-crops-137-0.52.hdf5", input_shape)
...
plt.figure(1); plt.imshow(x); plt.show(block=False)
aa = prediction.squeeze()
plt.figure(2); plt.imshow(aa[:,:,0]); plt.title('adult_males'); plt.show(block=False)
plt.figure(3); plt.imshow(aa[:,:,4]); plt.title('pups'); plt.show()
